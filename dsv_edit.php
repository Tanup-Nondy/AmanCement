	...
	<div class="row">
		<div class="col-md-12">
            <div class="input-group" id="adv-search">
                <input type="text" class="form-control" placeholder="Search for District Sales Volume" />
...
if(isset($_POST['submit'])){
	...
	echo '<th style="color:black;">Sales</th>';
	echo '<th style="color:black;">Year</th>';
	echo '<th style="color:black;">Month</th>';
	echo '<th style="color:black;">Remarks</th>';
	echo '<th style="color:black;" colspan="3"><center>Actions</center></th>';
	echo '</tr>';
	$s="SELECT DISTINCT `Brand` FROM `dsv`";
	$q=mysqli_query($cn,$s);
	if($q){
	    while($row2=mysqli_fetch_array($q)){
	        $b=$row2['Brand'];
	foreach($month AS $m){
	$search="SELECT `SI_NO`, `Brand`, `S_V`, `year`, `month`, `Remarks`, `district` FROM `dsv` WHERE year='$year' AND district='$dis' AND month='$m' AND Brand='$b'";
	...
	while($row=mysqli_fetch_array($chk)){
	   ...
	   echo '<td style="color:black;">'.$row['S_V'].'</td>';
	   echo '<td style="color:black;">'.$row['year'].'</td>';
	   echo '<td style="color:black;">'.$row['month'].'</td>';
